...
const ProductCreateView = () => {
  ...
  useEffect(() => {
    ...
    const fetchMetadata = async () => {
      ...
      try {
        const res = await fetch('/api/products/metadata', { signal: controller.signal });
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 400 && typeof data === 'object' && !data.error && !data.message) {
           ...
           setGeneralError(data.error || data.message || text);
        }
        return;
      }
      ...
      clearTimeout(timeoutId);
      const errMsg = err.name === 'AbortError' ? 'The request timed out. Please check your connection.' : (err.message || 'An error occurred while creating the product');
      setGeneralError(errMsg);
    } finally {
      setSubmitting(false);
    }
